from flask_wtf import FlaskForm #csrf_tokenで乱数が自動設定される。
...
#ログイン用のForm
class LoginForm(FlaskForm): #htmlから取得したform情報が引数となる。
                       #引数はFormの型を使う。→正確には、親クラスのFormを継承している。
    email = StringField(
        'メール：', validators=[DataRequired(), Email()]  #varidatorsとは入力内容チェックを示している。
                                                        # DataRequiredは必須チェック項目を示す。
                                                        #Emailはメールアドレスの値になっているかをチェック
                                                        #EqualToは入力内容が同じかどうか
                                                        #(パスワード2つ入力させて双方が同じかチェックするとか）
    )
...
class ForgotPasswordForm(FlaskForm):
    email = StringField('メール：　',validators=[DataRequired(), Email()]) #HTMLファイルから取得したメールアドレスの情報を入れる型。
...
class UserForm(FlaskForm):
    ...
    def validate(self): #メールアドレスが既に存在していないかを確認する。
